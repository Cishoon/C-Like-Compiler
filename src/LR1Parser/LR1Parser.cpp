...
LR1Parser::LR1Parser(const std::string &file_path) {
    ...
    if (!std::getline(file, first_line)) {
        std::cerr << "文件格式错误: 首行必须定义起始符和终止符" << std::endl;
    }
...
void LR1Parser::parse_EBNF_line(const std::string &line) {
    ...
    while (std::getline(rhsStream, token, '|')) {
        ...
        while (tokenStream >> sym) {
            ...
            rhsSymbols.emplace_back(type, sym);
        }
        // 创建产生式并添加到某个容器中
        productionMap[Symbol(SymbolType::NonTerminal, lhs)].emplace_back(Symbol(SymbolType::NonTerminal, lhs),
                                                                         rhsSymbols);
        productions.emplace_back(Symbol(SymbolType::NonTerminal, lhs), rhsSymbols);
    }
}
...
void LR1Parser::calculate_firstSets() {
    ...
    do {
        changed = false;
        for (const auto &[nonTerminal, _productions]: productionMap) {
...
std::unordered_set<Symbol, SymbolHash, SymbolEqual>
LR1Parser::get_first_string(const std::vector<Symbol> &content) const {
    ...
    while (changed) {
        ...
        for (const auto &item: old_set) {
            ...
            for (const auto &prod: next_prods) {
                std::unordered_set<Symbol, SymbolHash, SymbolEqual> lookaheads = get_first_string(
                        {nnext_symbol, item.lookahead});
...
std::vector<Production> LR1Parser::get_productions_start_by_symbol(const Symbol &symbol) const {
    ...
    if (it == productionMap.end()) return {};
    return it->second;
}
...
void LR1Parser::construct_tables() {
    ...
    lr1ItemSets.push_back(
            std::unordered_set<LR1Item, LR1ItemHash, LR1ItemEqual>({LR1Item(begin_production, 0, end_symbol)}));
...
bool LR1Parser::parse(const std::vector<Symbol> &sentence) {
    stateStack.push(0);
...
Action LR1Parser::get_next_action(const Symbol &currentSymbol) {
    size_t currentState = stateStack.top();
